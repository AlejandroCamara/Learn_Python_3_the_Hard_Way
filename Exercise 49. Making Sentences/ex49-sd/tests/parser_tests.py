...
def test_peek():
    # Nothing to return
    empty_word_list = []
    assert_equal(None, test_parser.peek(empty_word_list))

    # Return word_list[0][0]
    word_list = [('noun','test'), ('direction', 'north')]
    assert_equal('noun', test_parser.peek(word_list))
    

def test_match():
    # Nothing to match
    empty_word_list = []
    assert_equal(None, test_parser.match(empty_word_list, None))

    # word_list[0][0] word type mismatch
    word_list = [('something','test'), ('direction', 'north')]
    assert_equal(None, test_parser.match(word_list, 'noun'))

    # Right word_list[0][0] word type match
    word_list = [('noun','test'), ('direction', 'north')]
    assert_equal(('noun', 'test'), test_parser.match(word_list, 'noun'))
    

def test_skip():
    # Pop all the word_list elements
    word_list = [('stop','the'), ('stop', 'of'), ('stop', 'all')]
    test_parser.skip(word_list, 'stop')
    assert_equal([], word_list)

    # Pop first element
    word_list = [('noun','Test'), ('stop', 'of'), ('direction', 'West')]
    test_parser.skip(word_list, 'noun')
...
def test_parse_verb():
    ...
    assert_equal(('verb','run'), test_parser.parse_verb(word_list))
...
def test_object():
    ...
    assert_equal(('noun','force'), test_parser.parse_object(word_list))

    # Direction as an object 
    word_list = [('direction','north'), ('noun','force')]
    assert_equal(('direction','north'), test_parser.parse_object(word_list))
    ...
    word_list = [('verb','run')] 
    test_parser.parse_object(word_list)

@raises(parser.ParserException)
def test_subject():
    # No noun
    word_list = [('verb','run'), ('direction', 'north')]
    assert_equal(('noun','player'), test_parser.parse_subject(word_list))

    # With noun
    word_list = [('noun', 'Princess'),('verb','run'), ('direction', 'north')]
    assert_equal(('noun','Princess'), test_parser.parse_subject(word_list))

    # No noun neither verb
    test_parser.parse_subject(('direction', 'north'))

@raises(parser.ParserException)
def test_sentence():
    # Default subject player 
    sentence = parser.Sentence(('noun','player'), ('verb','run'), ('direction','north'))
    parsed_sentence = test_parser.parse_sentence([('verb', 'run'), ('direction', 'north')])
    ...
    parsed_sentence = test_parser.parse_sentence([('noun', 'bear'), ('verb', 'eat'),
                                            ('stop', 'the'), ('noun', 'honey')])
    assert_equal([sentence.subject, sentence.verb, sentence.object], 
                [parsed_sentence.subject, parsed_sentence.verb, parsed_sentence.object])

    # Wrong sentence
    test_parser.parse_sentence([('direction', 'south'), ('verb', 'run')])

    # Wrong sentence
    test_parser.parse_sentence([('noun', 'The'), ('direction', 'south'), ('verb', 'run')])
